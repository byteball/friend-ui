...
export const GovernanceDepositAssetItemContent: FC<GovernanceDepositAssetItemContent> = ({
  ...
  return <CardContent>
    {leaderValue ? <div suppressHydrationWarning className="flex items-center justify-between">
      <div>Leader: {leaderValue === "no" ? <span>against this asset</span> : <a target="_blank" className="link-style" href={getExplorerUrl(leaderValue, "address")}>{truncateAddress(leaderValue)}</a>}
      </div>
